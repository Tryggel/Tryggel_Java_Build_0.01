...
def DevicesWattNow (Authorization):
	koll_url_live = 'https://api.eon.se/eonapi/ODataProvider.svc/KwStreamLive'	
	url = koll_url_live
	try:
		res = requests.get(
			url
...
def GettingData():
	...
	list_devices = [341934,341933,341932]
	list_token = ['Mi45MDcxOlRyeWdnZWwx']
		...
		while i < len(list_token): 
			...
			if w <> 0:
				...
				for device in w:
					...
					print ("Yes!")
					if device['deviceId'] in list_devices:
						print ("Yes!")
